 ...
 *
 * @param stream
 *   The validated single-output stream (rows carry `_dq_errors` / `_dq_skipped` fields).
 * @param errorTag
...
class ValidatedFlinkStream(
   ...
   *
   * @return
   *   A `(good, bad)` tuple of `DataStream[Row]`.
   */
   ...
   *
   * @return
   *   The enriched `SingleOutputStreamOperator[Row]`.
   */
