...
object FlinkValidator {
   ...
   *
   * Note: `_dq_errors` is a JSON string carrying `rule_id`, `check_type`, `field`, `category`, `expected`, `actual`,
   * `message`, and `timestamp` — the same eight fields, same order, as the Spark `array<struct<...>>`, but serialized
   ...
   *
   * @param stream
   *   The input `DataStream[Row]`, whose type must be a [[RowTypeInfo]].
   * @param rules
   *   Rules to apply.
   * @return
   *   A [[io.galileostd.sumeh.flink.validation.ValidatedFlinkStream]] whose `split()` exposes the `(good, bad)`
   *   streams.
   * @throws java.lang.IllegalArgumentException
   *   when `stream` does not carry an explicit `RowTypeInfo` (e.g. an inferred `GenericTypeInfo[Row]`).
   */
  def validate(
