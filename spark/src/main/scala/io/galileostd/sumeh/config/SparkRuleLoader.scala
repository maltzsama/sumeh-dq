...
object SparkRuleLoader {
   ...
   *
   * Required columns: field, check_type
   * Optional: value, threshold, execute, level, category
   * Extra columns → metadata
   *
   * Example:
   *   val df = spark.read.table("dq_rules")
   *   val rules = SparkRuleLoader.fromDataFrame(df)
   *   val report = SparkValidator.validate(data, rules)
   */
  def fromDataFrame(df: DataFrame): List[RuleDefinition] = {
    ...
    df.collect().map { row =>
      val map = df.columns.map { col =>
        col -> Option(row.getAs[Any](col)).map(_.toString).getOrElse("")
      }.toMap
  ...
  /**
   * Load rules from a Spark DataFrame that has a single JSON column.
   * Each row is a rule object.
   *
   * Example:
   *   val df = spark.read.table("dq_rules_json")
  ...
  def fromJsonColumn(df: DataFrame, column: String = "config"): List[RuleDefinition] = {
    ...
    df.select(column).collect().map { row =>
