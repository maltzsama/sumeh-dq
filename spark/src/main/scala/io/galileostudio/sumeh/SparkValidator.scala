package io.galileostudio.sumeh.spark

import java.util.UUID

import io.galileostudio.sumeh.rule.RuleDefinition
import io.galileostudio.sumeh.spark.registry.SparkRegistry
import io.galileostudio.sumeh.validation.{ ValidationLevel, ValidationReport, ValidationResult, ValidationStatus }
...
object SparkValidator {
  ...
  private def buildFailCondition(df: DataFrame, rule: RuleDefinition) = {
    ...
    import io.galileostudio.sumeh.rule._
  ...
  private def ruleValueToAny(v: Option[io.galileostudio.sumeh.rule.RuleValue]): Any = v match {
    case Some(io.galileostudio.sumeh.rule.StringValue(s)) => s
    case Some(io.galileostudio.sumeh.rule.LongValue(l))   => l
    case Some(io.galileostudio.sumeh.rule.DoubleValue(d)) => d
    case _                                                => null
  }

  private def listValues(v: Option[io.galileostudio.sumeh.rule.RuleValue]): Seq[Any] = v match {
    case Some(io.galileostudio.sumeh.rule.ListValue(items)) => items.map(v => ruleValueToAny(Some(v)))
    case _                                                  => Seq.empty
  }
