package io.galileostudio.sumeh

import io.galileostudio.sumeh.spark.SparkValidator
import io.galileostudio.sumeh.rule.RuleDefinition
import io.galileostudio.sumeh.validation.ValidationStatus
...
class SparkValidatorSpec extends AnyWordSpec with Matchers with BeforeAndAfterAll {
  ...
  "Comparison" should {
    ...
    "pass is_greater_than 0" in {
      import io.galileostudio.sumeh.rule.LongValue
      val rules = Seq(RuleDefinition.validated(Left("age"), "is_greater_than", value = Some(LongValue(0)), threshold = 1.0))
      val report = SparkValidator.validate(dfBasic, rules)
      report.results.head.status shouldBe ValidationStatus.PASS
    }
    ...
    "fail is_greater_than 18 when age=17 exists" in {
      import io.galileostudio.sumeh.rule.LongValue
      val rules = Seq(RuleDefinition.validated(Left("age"), "is_greater_than", value = Some(LongValue(18)), threshold = 1.0))
      val report = SparkValidator.validate(dfBasic, rules)
      report.results.head.status shouldBe ValidationStatus.FAIL
    }

    "pass is_between [0, 150]" in {
      import io.galileostudio.sumeh.rule.{ListValue, LongValue}
      val rules = Seq(RuleDefinition.validated(Left("age"), "is_between",
        value = Some(ListValue(List(LongValue(0), LongValue(150)))), threshold = 1.0))
      val report = SparkValidator.validate(dfBasic, rules)
      report.results.head.status shouldBe ValidationStatus.PASS
    }
  ...
  "Membership" should {
    ...
    "pass is_contained_in with valid set" in {
      import io.galileostudio.sumeh.rule.{ListValue, StringValue}
      val rules = Seq(RuleDefinition.validated(Left("status"), "is_contained_in",
    ...
    "pass not_contained_in with absent values" in {
      import io.galileostudio.sumeh.rule.{ListValue, StringValue}
      val rules = Seq(RuleDefinition.validated(Left("status"), "not_contained_in",
  ...
  "Pattern" should {
    ...
    "pass has_pattern with threshold allowing nulls" in {
      import io.galileostudio.sumeh.rule.StringValue
      val rules = Seq(RuleDefinition.validated(Left("name"), "has_pattern",
        value = Some(StringValue("^[a-z]+$")), threshold = 0.7))
      val report = SparkValidator.validate(dfBasic, rules)
      report.results.head.status shouldBe ValidationStatus.PASS
    }
  ...
  "Aggregation" should {
    ...
    "pass has_mean with correct value" in {
      import io.galileostudio.sumeh.rule.DoubleValue
      val rules = Seq(RuleDefinition.validated(Left("age"), "has_mean",
        value = Some(DoubleValue(29.4)), threshold = 0.0))
      val report = SparkValidator.validate(dfBasic, rules)
      report.results.head.status shouldBe ValidationStatus.PASS
    }
    ...
    "pass has_min" in {
      ...
      val rules = Seq(RuleDefinition.validated(Left("age"), "has_min",
        value = Some(LongValue(17)), threshold = 0.0))
      val report = SparkValidator.validate(dfBasic, rules)
      report.results.head.status shouldBe ValidationStatus.PASS
    }
    ...
    "pass has_max" in {
      import io.galileostudio.sumeh.rule.LongValue
      val rules = Seq(RuleDefinition.validated(Left("age"), "has_max",
        value = Some(LongValue(40)), threshold = 0.0))
      val report = SparkValidator.validate(dfBasic, rules)
      report.results.head.status shouldBe ValidationStatus.PASS
    }
    ...
    "pass has_cardinality" in {
      import io.galileostudio.sumeh.rule.LongValue
      val rules = Seq(RuleDefinition.validated(Left("status"), "has_cardinality",
        value = Some(LongValue(3)), threshold = 0.0))
      val report = SparkValidator.validate(dfBasic, rules)
      report.results.head.status shouldBe ValidationStatus.PASS
    }
  }
